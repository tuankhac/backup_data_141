...
<%
	...
	data.put(11, request.getParameter("ma_nv"));
	...
	CallableStatement cstm = null;
  	try {
		...
		cstm = conn.prepareCall("begin ?:= REPORT_NEW.GET_XML_UNT_NV(?,?,?,?,?,?,?,?,?,?,?,?,?) ;end;");
		...
		if(!sb.toString().startsWith("Loi thuc hien")){
			...
			result = "Err: Kiểm tra REPORT_NEW.GET_XML_UNT_BLU hoặc tham số.";
		}
	} catch (Exception e) {
		e.printStackTrace();
 	}	
