...
is
	s varchar2(1000);
begin
	logger.access('tax.phieu_q42015|NEW',pid||'|'||pmst||'|'||ptien_giao||'|'||pma_nv||'|'||pngay_giao||'|'||pnguoi_giao);
	s:='insert into tax.phieu_q42015(id,mst,tien_giao,ma_nv,ngay_giao,nguoi_giao)
	 values (:id,:mst,:tien_giao,:ma_nv,:ngay_giao,:nguoi_giao)';
	execute immediate s using pid,pmst,ptien_giao,pma_nv,to_date(pngay_giao,'dd/mm/yyyy'),pnguoi_giao;
	commit;return '1';
...
is
	...
	ref_ sys_refcursor;
begin
	logger.access('tax.phieu_q42015|SEARCH',pid||'|'||pmst||'|'||ptien_giao||'|'||pma_nv||'|'||pngay_giao||'|'||pnguoi_giao);
...
is
	s varchar2(1000);
begin
	logger.access('tax.phieu_q42015|EDIT',pid||'|'||pmst||'|'||ptien_giao||'|'||pma_nv||'|'||pngay_giao||'|'||pnguoi_giao);
	s:='update tax.phieu_q42015 set  id=:id,mst=:mst,tien_giao=:tien_giao,ma_nv=:ma_nv,ngay_giao=:ngay_giao,nguoi_giao=:nguoi_giao where id=:id';
	execute immediate s using pid,pmst,ptien_giao,pma_nv,to_date(pngay_giao,'dd/mm/yyyy'),pnguoi_giao,pid;
	commit;return '1';
	exception when others then
		declare	err varchar2(500); begin	err:='Loi thuc hien, ma loi:'||to_char(sqlerrm); return err;
	end;
