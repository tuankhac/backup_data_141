...
<!-- Content Wrapper. Contains page content -->
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <h1>
            ${n.i18n.gui_email_thu_thue}
          </h1>
        </section>
        <section class="content">
 			<div class="nav-tabs-custom">
            <ul class="nav nav-tabs">
              <li class="active"><a href="#tab_1" data-toggle="tab" aria-expanded="true">${n.i18n.gui_email_thu_thue_theo_lo}</a></li>
              <li class=""><a href="#tab_2" data-toggle="tab" aria-expanded="false">${n.i18n.gui_email_thu_thue_nhan_cong}</a></li>
				...
				<!-- <form role="form" id="form_tab1"> -->
	              <div class="box-body">
	                <div class="form-group">
	                  	<div class="form-group">
		                  <label>${n.i18n.gui_email_thu_thue_theo_lo_ma_tinh}</label>
					...
					<div class="form-group">
						...
						<input type="text" class="form-control" id="subject1" name="subject1" placeholder="Tiêu đề email">
					</div>								
	                <div class="form-group">
	                  <label for="noi_dung_email">${n.i18n.gui_email_thu_thue_theo_lo_noi_dung}</label>
	                  <textarea type="text" class="form-control" id="noi_dung_email1" placeholder="${n.i18n.gui_email_thu_thue_theo_lo_noi_dung}" rows="3" ></textarea>
	                </div>

	              </div>
	              <!-- /.box-body -->
	              <div class="box-footer">
	                <button  class="btn btn-primary" onclick="doSend1()">${n.i18n.gui_email_thu_thue_btn_gui}</button>
	                <font id="result"></font>
	              </div>
	            <!-- </form> -->
              </div>
	              	...
	              	<div class="box-body no-padding" style="overflow:auto;margin:auto">
						<table class='table table-condensed table-striped table-bordered' width="100%">
							...
							<tr>
								...
								<td>
					                <div class="form-group">
					                  	<label for="ds_stb">${n.i18n.gui_email_thu_thue_lbl_file_sdt}</label>
						...
						<div class="form-group">
							<label for="subject">Tiêu đề</label>
							<input type="text" class="form-control" id="subject2" name="subject2" placeholder="Tiêu đề email">
						</div>
		                <div class="form-group">
		                  <label for="noi_dung_email">${n.i18n.gui_email_thu_thue_theo_lo_noi_dung}</label>
		                  <textarea type="text" class="form-control" id="noi_dung_email2" placeholder="${n.i18n.gui_email_thu_thue_theo_lo_noi_dung}" rows="3"></textarea>
		                </div>
	              	</div>
	              <!-- /.box-body -->
	              <div class="box-footer">
	                <button type="button" onClick="doSend2();" class="btn btn-primary">${n.i18n.gui_email_thu_thue_btn_gui}</button>
...
<script>
	...
	function doSend1(){
		...
		var exec= '&receiver='+$('#ma_tinh').val()+'&content='+$('#noi_dung_email1').val()+'&subject='+$('#subject1').val()+'&province_id='+$('#ma_tinh').val()+'&created_user=${pageContext.request.userPrincipal.name}'+'&created_date='+'&type=EMAIL'+'&userid=${pageContext.request.userPrincipal.name}&userip=$!{request.RemoteAddr}';
		$('#result').html('${n.i18n.crud_process_message}');
		$.ajax({
			url: 'crud_exec.jsp?crud_type=notification_log/new_exec.html'+exec,
			success: function(data){ 
				if (data=="1.0"){
					$('#result').html("${n.i18n.gui_email_thanh_cong}"); 
				}else{
					$('#result').html(data);
				}
			}
	...
	function uploadFileAttach(){
		...
		var u = 'gui_email/uploadfile.jsp';
		var dsSoThueBao="";
		//alert(u);
		// loading();
		$.ajax({
	...
	function doSendNhanCong(stb){
		var exec= '&receiver='+stb.trim()+'&content='+$('#noi_dung_email2').val()+'&subject='+$('#subject2').val()+'&province_id='+$('#ma_tinh').val()+'&created_user=${pageContext.request.userPrincipal.name}'+'&created_date='+'&type=EMAIL'+'&userid=${pageContext.request.userPrincipal.name}&userip=$!{request.RemoteAddr}';
		$('#result2').html('${n.i18n.crud_process_message}');
		$.ajax({
			url: 'crud_exec.jsp?crud_type=notification_log/new_exec.html'+exec,
			success: function(data){ 
				if (data=="1.0"){
					$('#result2').html("${n.i18n.gui_email_thanh_cong}"); 
				}else{
					$('#result2').html(data); 
				}
			}
