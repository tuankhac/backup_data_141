...
<%
	...
	String realPath = "tong_hop_du_lieu_thue" +"/du_lieu_thanhtoan_download/";
	String rootPath = application.getRealPath("/")+ realPath;
	String fileName = "dulieuthanhtoan";
	...
	try{
		...
		cstm = conn
			.prepareCall("begin ?:= upload.search_uploads_thanhtoan_temp(?,?,?,?,?,?) ;end;");
		cstm.registerOutParameter(1, OracleTypes.CURSOR);
		cstm.setString(2,filter);
		cstm.setString(3,request.getUserPrincipal().getName());
		cstm.setString(4,request.getRemoteAddr());
		cstm.setString(5,currId);
		cstm.setString(6,"1");
		cstm.setString(7,"-2");
		cstm.execute();
		...
		if (totalColumn > 0) {
			columnNames = new String[totalColumn];
		...
		if (outputType.equals("EXCEL")) {
			result = doWriteExcel(listData, rootPath, fullName + ".xls", 24, columnNames);
 		} else {
			result = doWriteDBF(listData, rootPath, fullName + ".dbf", 24, columnNames);
		}
	}catch(Exception e){
		//e.printStackTrace();
 		result = e.getMessage();
