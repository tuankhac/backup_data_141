...
<%!
	...
	public static String insertXlsData(
		...
		String result = "";
		String sql = "insert into tax.thanhtoan_upload_temp(ma_cqt,ma_goi_tin,ma_kho_bac,ngay_kho_bac,mst,ten_nnt,diachi_nnt,so_qd,so_lenh_hoan,ngay_lenh_hoan,mcq,tk_no,tk_co,tien_te,ty_gia,so_chung_tu,nien_do,chuong,khoan,tieu_muc,ky_thue,so_tien,ngay_tt,trang_thai,upload_id,log_user,log_date,log_ip) "
				+ " values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,sysdate,?)";
		...
		try {
			...
			for (Iterator itr = myMap.entrySet().iterator(); itr.hasNext();) {
				...
				ps.setString(24, record.containsKey(24) ? record.get(24) : "0");
				ps.setLong(25, mySEQ);
				ps.setString(26, mParams.get("log_user"));
 				ps.setString(27, mParams.get("log_ip"));
				ps.addBatch();
				count++;
				if (count >= batchSize) {
					count = 0;
