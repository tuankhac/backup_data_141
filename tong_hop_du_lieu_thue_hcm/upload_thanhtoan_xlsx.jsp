	...
	try {
		...
		if(file_name == null || file_name.length() == 0){
			...
			if(extFileName.equals(".xls") && UPLOAD_FILE_TYPE == 0){
				Map<Integer, HashMap<Integer, String>> dataXls = readXLS(rootFolder+duLieuThanhToanFolder + file_name,mXlsColumnName,SHEET_NUMB,excelType);
				if(dataXls == null  ){
...
%>
<%!
	public static Map<Integer, HashMap<Integer, String>> readXLS(String path,String[] mXlsColumnName, int numbOfSheet, int excelType) {
		...
		try {
			if(excelType == 0){
				...
				while (rows.hasNext()) {
					...
					int i = 1;
					record = new HashMap<Integer, String>();
					for (int i = 1; i <= mXlsColumnName.length; i++) {
						...
						if (cell == null) {
							cell = row.createCell(j-1, XSSFCell.CELL_TYPE_BLANK);
							record.put(objKey, "");
						} else {
							record.put(objKey, getCellXSSFValue(cell));
						}
					}
					myMap.put(key, record);
					key++;
