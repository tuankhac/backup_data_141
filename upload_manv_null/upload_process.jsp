...
<%!
	...
	public static String insertXlsData(Map<Integer, String[]> myMap,
		...
		String sql = "SELECT TBL_TEMP_SEQ.NEXTVAL FROM DUAL";
		...
		try {
			...
			if(rs.next())
				seq = rs.getString(1);
			sql = "INSERT INTO UPLOAD_MST_MANV_TEMP (ID,MST,MA_NV,MA_CQT,MA_TINH,USER_ID,IP,LOGDATE,STATUS,LOG_MSG,ID_TABLE) VALUES(TABLE_TEMP_SEQ.NEXTVAL,?,?,?,?,?,?,sysdate,0,'Insert tu excel',?)";
			...
			for (Iterator itr = myMap.entrySet().iterator(); itr.hasNext();) {
				...
				ps.setString(3, mParams.get("co_quan"));
				ps.setString(4, mParams.get("ma_tinh"));
				ps.setString(5, mParams.get("user"));
				ps.setString(6, mParams.get("user_ip"));
				ps.setString(7, seq);
				count ++;
				sum ++;
				ps.addBatch();
			...
			"<tr><td align=left>Số bản ghi hợp lệ :<span id=inserted_records></span></td></tr>"+
			"<tr><td align=left>Số bản ghi không hợp lệ : <span id=failed_records></span></td></tr></table></div>";
